...
let bookAge = currentYear - publicationYear;

if (bookStatus.toLowerCase() === "có sẵn" && bookAge <= 5) {
    console.log(`Sách này mới và có sẵn để mượn.`);
}
else if (bookStatus.toLowerCase() === "đã mượn" && bookAge < 10) {
    console.log(`Sách này đã mượn nhưng khá mới, có thể mượn lại sau.`);
}
else if (bookStatus.toLowerCase() === "đã mượn" && bookAge > 10) {
    console.log(`Sách này đã mượn và khá cũ.`);
}
else if (bookStatus.toLowerCase() === "có sẵn" && bookAge > 5) {
